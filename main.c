...
int main(void) {
    ...
    // 3. ADIM: Görsel ve Ses Yükleme (InitWindow'dan SONRA)
    ...
    while (!WindowShouldClose()) {
        ...
        // Yolun akma mantığı (Görsel yüklendiyse)
        if (yolDokusu.id > 0) {
        ...
        // --- BU KISMI EKLE: Yuva ve Level Mantığı ---
        allFull = true;
        for (int i = 0; i < 5; i++) {
            if (CheckCollisionRecs((Rectangle){frogPos.x, frogPos.y, (float)frogImage.width, (float)frogImage.height}, finishZones[i]) && !zoneOccupied[i]) {
                ...
                score += 100;
                frogPos = (Vector2){ (float)screenWidth/2 - (float)frogImage.width/2, (float)screenHeight - (float)frogImage.height - 20 };
                gameTimer = maxTimer; // Süreyi sıfırla
            }
        ...
        // GÜVENLİ YOL ÇİZİMİ
        DrawRectangle(0, 320, 800, 200, GRAY); // Yedek gri zemin
        if (yolDokusu.id > 0) {
            // Görsel varsa döşeme yap
        ...
        // Şeridi yolun akışıyla senkronize et
        for (int i = -1; i < 10; i++) {
